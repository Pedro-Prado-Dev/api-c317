...
class TestUserResource(unittest.TestCase):
    ...
    def test_update_user(self):
        user = User(
            name="Update User", phone_number="555555555", email="updateuser@example.com", role="user", is_adm=False
        )
        user.set_password("password")
        db.session.add(user)
        db.session.commit()
    ...
    def test_delete_user(self):
        user = User(
            name="Delete User", phone_number="333333333", email="deleteuser@example.com", role="user", is_adm=False
        )
        user.set_password("password")
        db.session.add(user)
        db.session.commit()
